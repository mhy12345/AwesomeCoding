				...
				it('file_table.',function() {
					return getConnection().
						...
						then(function(packed) {
							let {conn, sql_res} = packed;
							conn.end();
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('id');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('user_id');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('filename');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('type');});
						});
				});
				it('resources.',function() {
					return getConnection().
						...
						then(function(packed) {
							...
							conn.end();
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('id');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('class_id');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('resource');});
						});
				});
				it('classes.',function() {
					return getConnection().
						...
						then(function(packed) {
							...
							conn.end();
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('id');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('description');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('notice');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('title');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('registration_date');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('invitation_code');});
						});
				});
				it('classusers.',function() {
					return getConnection().
						...
						then(function(packed) {
							...
							conn.end();
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('id');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('role');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('userid');});
							sql_res.results.should.matchAny((val) => {val.COLUMN_NAME.should.be.eql('registration_date');});
						});
				});
			});
